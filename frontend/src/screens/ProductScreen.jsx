...
const ProductScreen = () => {
  ...
  const {error:errorProductReview, success: successProdcutReview } = productReviewCreate;
  ...
  useEffect(()=>{
    if(successProdcutReview)
    {
      ...
      setRating(0);
      setComment('');
      dispatch({typle: PRODUCT_CREATE_REVIEW_RESET});
    }
   dispatch(listProductDetails(id));

  },[dispatch,successProdcutReview])
  ...
  return (
    ...
    <Row>
      <Col md={6}>
        <Image src={product.image} alt={product.name} style={{ height: "500px", maxWidth:"650px"}} />
      </Col>
      <Col md={3}>
      ...
      <Col md={3}>
        <Card>
          <ListGroup variant='flush'>
            ...
            <ListGroup.Item>
              <Row>
              <Button className='btn-block' type='button' disabled={product.countInStock === 0} onClick = {addToCartHander}>
                Add To Cart
              </Button>
              </Row>
            </ListGroup.Item>
